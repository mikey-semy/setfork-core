...
/// Размер мини-пула под advisory-локи репо (SETFORK_LOCK_POOL_MAX, дефолт 4).
///
/// Это ЕДИНСТВЕННЫЙ в ядре глобальный потолок одновременных git-операций: гард
/// держит соединение на всю операцию, значит одновременных операций по ВСЕМ
/// спискам не больше, чем соединений в этом пуле; следующий ждёт `acquire_timeout`.
/// Хардкод мешал даже ПРОВЕРИТЬ гипотезу «упираемся ли мы в него» — а замер шага 1
/// трека производительности показал, что сегодня не упираемся (17 вызовов RPC за
/// полтора часа). Значит величина остаётся прежней, но становится настраиваемой:
/// поднять её на час нагрузки теперь стоит переменной окружения, а не выката.
///
/// Ленивый аксессор по той же причине, что `repo_limit_bytes`.
pub fn lock_pool_max() -> u32 {
    static V: std::sync::OnceLock<u32> = std::sync::OnceLock::new();
    *V.get_or_init(|| match env_u32("SETFORK_LOCK_POOL_MAX", 4) {
        0 => {
            tracing::warn!("SETFORK_LOCK_POOL_MAX=0 means no git operations at all, using 4");
            4
        }
        v => v,
    })
