...
///
/// Решение владельца 23.09.2026 (исследование `2026-09-23-agent-skills`, §4):
/// ТЕКСТ скилла живёт в git-дереве списка, бинарь — в S3 по хешу. Причина — версия
/// обязана доказывать, что в ней лежит: скрипт вне дерева меняется, не меняя SHA,
/// и прошедший ревью скилл начинает делать другое (AST05 OWASP Agentic Skills
/// Top 10). В дереве же каждый байт скрипта покрыт SHA версии, виден в диффе и
...
/// Это скилл, а не хранилище: больше — признак того, что в дерево уезжает не то
/// (зависимости, сборка, данные), а у каждого списка свой bare-репозиторий,
/// который ещё и зеркалится.
...
/// дешевле правила, которое придётся ужесточать на уже принятых деревьях.
/// Пустое имя и имена-точки (`.`, `..`) отвергаются так же, как у шелльного
/// близнеца в хуке (`[^/]+` плюс отказ git принимать `..` в дереве).
pub fn authored_path(path: &str) -> bool {
    AUTHORED_DIRS.iter().any(|dir| {
