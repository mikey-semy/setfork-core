...
const PRE_RECEIVE_BODY: &[&str] = &[
    ...
    // ПРОСТРАНСТВО ИМЁН ПОСТОРОННЕГО (Ф5). Владелец и соавтор пушат куда угодно —
    // их право уже проверил фронт. Посторонний — только в своё `u/<ник>/*` и в
    // `refs/for/main`, который в это же пространство и ложится.
    //
    // ⚠️ Ограничиваем ровно при ЯВНОЙ роли `contributor`, а не «при всём, что не
    // владелец». Строгое умолчание я поставил первым и ошибся: оно кладёт мину в
    // выкатку. Ядро приезжает на прод раньше или позже фронта, и в окно «новое
    // ядро ↔ старый фронт» роли не приходит вовсе — тогда строгое умолчание
    // объявляет ПОСТОРОННИМ ВСЕХ, включая владельца, и пуш в main перестаёт
    // работать у всех сразу (авто-ревью core#80).
    //
    // Дыры это не открывает: пустая роль приходит только от НАШЕГО фронта, канал
    // до ядра закрыт токеном (ADR-0011 §2), а фронт, который не умеет ролей, и
    // посторонних не впускает — он их отвергал до Ф5 и продолжает отвергать.
    // Иначе говоря, «нет роли» означает «фронт старый», а не «неизвестно кто».
    // Чтобы это не стало тихим состоянием, ядро пишет о нём в лог (git_core.rs).
    ...
    "        msg contributor_namespace_hint \"$actor\" >&2",
    "        exit 1",
    "        ;;",
    "    esac",
    "  fi",
