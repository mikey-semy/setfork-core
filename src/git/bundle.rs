...
const PRE_RECEIVE_BODY: &[&str] = &[
    ...
    //
    // ⚠️ Пустая роль = «посторонний», а НЕ «свой». Забытый параметр обязан
    // закрывать дверь, а не открывать: ровно этот риск назван в треке причиной
    // делать Ф5 после Ф1.
    "role=\"${SETFORK_ROLE:-contributor}\"",
    ...
    //
    // Сравнение с префиксом безопасно ровно потому, что имена ASCII-ограничены
    // (`valid_branch`, линза 02 F5): будь тут юникод, `u/аdmin` с кириллической
    // «а» читался бы как чужое пространство и правило обходилось бы двойником.
