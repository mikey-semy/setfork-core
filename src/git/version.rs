...
//! теги vN — указатели; Postgres можно пересобрать из git (reproject).
//!
//! ⚠️ Обратное направление — «git из Postgres» — НЕ запрещено, а ОГРАНИЧЕНО, и
//! путать эти два утверждения дорого. Сборка репо из БД (`bootstrap_bare`)
//! случается ровно в двух местах: репозитория нет на диске, и репозиторий есть,
//! но в нём нет ни main, ни единого тега версии. Всё остальное лечится ВОЗВРАТОМ
//! ССЫЛКИ на существующий коммит. Причина жёсткая: `load_bundle_data` берёт
//! СЕГОДНЯШНЮЮ мету списка для ВСЕХ версий, поэтому пересборка после любой правки
//! названия даёт другие деревья и другие SHA, а теги vN форсом переезжают на эту
//! синтетику — вместе с потерей принятых пушей и коммитов слияния. Проверено
//! прогоном (линза 02, F10): при неизменной мете SHA совпадают и подмена
//! незаметна, при изменённой — расходятся все.
//!
