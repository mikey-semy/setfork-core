...
//!
//! В ветку автора `u/<id>/<base>` — ОДНУ на пару (автор, база). Имя
...
///
/// ⚠️ Имя строится по НЕИЗМЕННОМУ идентификатору автора, а не по нику.
///
/// Ник сменяем и после смены достаётся другому человеку — а имя ветки живёт
/// вечно. С ником в имени получалась цепочка: Алиса предъявила правку (ветка
/// `u/alice/main`), переименовалась, Борис взял ник `alice`, запушил — и попал в
/// ЧУЖУЮ ветку: предложение Алисы стало показывать содержимое Бориса, оставаясь
/// подписанным Алисой (авто-ревью fe#662).
///
/// Так же у всех, на кого стоит смотреть: у Gerrit ветки вклада зовутся
/// `refs/changes/NN/CCCC/PP`, у GitHub — `refs/pull/N/head`. Ни там, ни там ник
/// в имя не попадает — ровно потому, что имя обязано пережить переименование.
///
/// Ветку никто не набирает руками: человек пушит `refs/for/main`, а это имя
/// сервер придумывает сам. Читаемость тут не нужна, неизменность нужна.
pub fn author_branch(actor_id: &str, base: &str) -> String {
    format!("u/{actor_id}/{base}")
}
...
///
/// Пустой `actor_id` сюда не доходит — такой пуш отвергает хук, у которого есть
...
pub fn take_magic_pushes(
    ...
    for name in names {
        ...
        let branch = author_branch(actor_id, base);
