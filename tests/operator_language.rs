...
//!
//! ⚠️ ИСТОРИЯ, которую стоит знать. До 27.08 здесь жил названный СПИСОК ДОЛГА: пять
//! русских строк про зеркало, которые нельзя было просто перевести — они доезжают до
//! владельца списка, и английский показал бы англоязычный текст русскому человеку.
//! Список сам проверялся на живость: запись, которой в коде уже нет, роняла гейт, иначе
//! она вечно прикрывала бы новую такую же.
//!
//! Долг выплачен (И3): ядро отдаёт машинные коды `not-configured` / `secret-missing` /
//! `token-undecryptable`, текст подбирает фронт по языку человека. Проверка живости
//! сработала ровно тогда, когда была нужна — на первом же снятии строк, — и список убран
//! вместе с механикой. Знание оставлено здесь: если такая строка появится снова, ей место
//! не в списке исключений, а в коде причины.
//!
